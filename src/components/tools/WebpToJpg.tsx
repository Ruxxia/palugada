    ...
    <div className="space-y-6">
      <div className="border-2 border-dashed border-foreground/15 hover:border-foreground/30 rounded-xl p-8 text-center transition-colors relative cursor-pointer bg-foreground/5">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm font-medium">Klik atau drag file gambar WebP ke sini</p>
          <p className="text-xs text-foreground/45">Mengonversi WebP ke JPG secara offline</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <div className="space-y-6 bg-foreground/5 p-6 rounded-xl border border-foreground/10">
            ...
            <div className="flex justify-between items-center">
              <span className="text-sm font-medium">Warna Latar Belakang (Transparansi)</span>
              <input
                type="color"
                value={bgColor}
                onChange={(e) => setBgColor(e.target.value)}
                className="w-10 h-10 border border-foreground/15 rounded cursor-pointer"
              />
            </div>
            ...
            >
              {isProcessing ? "Memproses..." : "Konversi ke JPG"}
            </button>
