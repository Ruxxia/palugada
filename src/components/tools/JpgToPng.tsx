    ...
    <div className="space-y-6">
      <div className="border-2 border-dashed border-foreground/15 hover:border-foreground/30 rounded-xl p-8 text-center transition-colors relative cursor-pointer bg-foreground/5">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-foreground/45">Mengonversi JPG ke PNG secara offline</p>
        </div>
      </div>
      ...
      {selectedFile && previewUrl && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <div className="space-y-6 bg-foreground/5 p-6 rounded-xl border border-foreground/10">
            ...
            >
              {isProcessing ? "Memproses..." : "Konversi ke PNG"}
            </button>

            {convertedUrl && (
              <div className="space-y-4 pt-4 border-t border-foreground/10">
                <p className="text-xs text-foreground/50 font-mono">Gambar berhasil dikonversi ke format PNG tanpa kompresi kehilangan data.</p>
                <a
                  href={convertedUrl}
                  download={selectedFile.name.replace(/\.[^/.]+$/, "") + ".png"}
