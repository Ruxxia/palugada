  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Changelog — Palugada" },
      {
        ...
        content: "Daftar pembaruan dan tools baru di platform Palugada.",
      },
      ...
      { name: "twitter:title", content: "Changelog — Palugada" },
      {
        ...
        <main className="max-w-4xl mx-auto px-4 py-12">
          ...
          <div className="border-b-2 border-foreground pb-6 mb-12">
            ...
            <p className="text-foreground/60 text-lg leading-relaxed max-w-[60ch]">
              Catatan pembaruan, perbaikan bug, dan peluncuran tools gratis baru di Palugada. Kami
              terus menambahkan utilitas baru untuk membantu produktivitas harian Anda.
            </p>
          </div>
          ...
          <div className="space-y-12 relative before:absolute before:left-4 md:before:left-1/2 before:top-4 before:bottom-4 before:w-0.5 before:bg-foreground/10">
            {changelogData.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`relative flex flex-col md:flex-row gap-6 md:gap-12 group ${rowDirectionClass}`}
                >
                  {/* Time Indicator dot */}
                  <button
                    onClick={() => toggleExpand(idx)}
                    aria-label={isExpanded ? "Collapse section" : "Expand section"}
                    className={`absolute left-4 md:left-1/2 -translate-x-[7px] w-[15px] h-[15px] rounded-full z-10 top-1.5 transition-all duration-300 ${
                      isExpanded
                  ...
                  >
                    <span className="font-mono text-sm text-foreground/40 block mb-1 group-hover/date:text-primary transition-colors">
                      {item.date}{" "}
                      <span className="md:hidden text-xs text-primary/70 ml-2">
                        {isExpanded ? "▲ sembunyikan" : "▼ tampilkan"}
                      </span>
                    </span>
                    <h3
                      className={`font-display text-xl uppercase tracking-tight text-primary flex items-center gap-2 group-hover/date:opacity-90 transition-opacity ${titleClasses}`}
                    >
                      {item.title}
                    </h3>
                    <p
                      className={`text-sm text-foreground/60 mt-2 leading-relaxed md:max-w-[40ch] ${descClasses}`}
                    >
                      {item.description}
                    </p>
                  </div>
                  ...
                  {/* Content column */}
                  <div
                    className={`pl-10 space-y-4 transition-all duration-300 ${contentColClasses} ${
                      isExpanded
